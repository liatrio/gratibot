...
so redemption admins can view, add, edit, and soft-delete rewards —
...
- Let redemption admins view, add, edit, and soft-delete rewards through a
  single Slack modal opened by DMing or `@gratibot admin redeem`.
...
- **As a redemption admin**, I want to DM `admin redeem` to Gratibot and
  see a modal listing every reward so that I can review the current
  catalog without opening a shell or a PR.
...
- **As a redemption admin**, I want to soft-delete a reward with a
  confirmation step so that I can hide retired items from the end-user
  redeem list while preserving history and the original record.
...
- **As a non-admin end user**, I want to be told clearly that I am not
  authorized when I try `admin redeem` so that the system's boundary is
...
### Unit 2: Admin modal — authorization, list view, add, edit, soft-delete (no image upload yet)
...
- The system shall register a new Bolt message matcher that fires on the
  exact text `admin redeem` (case-insensitive, trimmed), in either a DM
  to the bot or a `@gratibot admin redeem` directed message in a
  channel.
- The system shall tighten the existing `redeem` matcher so that
  `admin redeem` does not also trigger the user-facing redeem flow. The
  `admin redeem` handler shall be the only handler that fires for that
  text.
- The system shall, when `admin redeem` is invoked by a user whose Slack
  ID is not in `config.redemptionAdmins`, respond with the exact text
  `You are not authorized to manage rewards.` and do nothing else.
- The system shall, when `admin redeem` is invoked by a redemption
  admin, open a modal whose main view lists every reward in the
  ...
  indicator, and an "Edit" button. An "Add new reward" button shall sit
...
- The system shall, when "Edit" is clicked on a row, replace the modal
  ...
  containing the same fields as Add plus a "Soft-delete" button.
  Submit shall persist updates to that document's editable fields and
  set `updatedBy` and `updatedAt`; Soft-delete shall set `active: false`
  after a confirmation prompt; both shall return the admin to the
  updated main view.
...
- `Test: test/service/rewardAdmin.js specs cover listRewards,
  createReward, updateReward, softDeleteReward, and isAuthorized`
  demonstrates CRUD and authorization logic is covered.
...
- `Screenshot: modal add-reward form, filled and submitted; modal
  ...
  path.
- `Screenshot: modal edit-reward form with Soft-delete confirmation;
  after confirm, main view shows the row as inactive` demonstrates the
  edit and soft-delete paths.
- `Slack: DM "admin redeem" from a non-admin account replies with
  "You are not authorized to manage rewards." and no modal opens`
  demonstrates the authorization boundary.
...
- The Edit form's "Soft-delete" button is red (Slack `style: "danger"`)
  and triggers a confirmation dialog. Confirmation text: "Hide this
  reward from the redemption list? Historical redemptions are not
  affected."
...
- `app.message` handler for `admin redeem` → authorization check →
  `client.views.open` with the main view built by the rewardAdmin
  service.
...
The existing `app.message(/redeem/i, ...)` in `features/redeem.js`
currently matches any message containing "redeem", including
"admin redeem". Tighten the user-facing matcher to exclude
`admin redeem` (e.g., negative lookahead or an explicit
`/^\s*redeem\b/i` combined with a rule that the new `admin redeem`
handler is registered with a more specific pattern and runs first).
The acceptance criterion is that exactly one handler fires per
incoming message for both the user and the admin trigger.
...
- **Authorization**: only `config.redemptionAdmins` users may open
  the admin modal or submit any admin modal view. The feature
  handler must re-check membership on every `view_submission` as
  well as on the initial `message` — not only at modal-open — so a
  non-admin cannot race the modal open or replay a view_submission
  payload.
...
1. **Zero deploys required for catalog changes**: after this spec
   ...
   via `admin redeem` with no GitHub PR and no container deploy.
...
3. **Admin flow reliability**: in a one-week nonprod bake period,
   ...
   soft-delete via the modal without an error requiring an engineer.
