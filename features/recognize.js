...
module.exports = function(controller) {
    controller.hears(emoji , ['direct_message', 'direct_mention', 'mention', 'message'], async (bot, message) => {
        ...
        const gratitudeRemaining = await balance.dailyGratitudeRemaining(userInfo.giver.id, userInfo.giver.tz);
...
async function testFunction(bot, message) {
    ...
    originalMessage = await bot.api.conversations.history({
...
async function userDetails(bot, message, giver) {
    ...
    const userInfo = {
        ...
        receivers: await Promise.all(userIds.map(async user => bot.api.users.info({user: user}))),
    };
...
//        userInfo.receivers.find(x => x.id == userInfo.giver.id) ? '- You can\'t recognize yourself' : '',
...
async function sendNotificationToReceivers(bot, message, userInfo) {
    let results = []
    for (let i = 0; i < userInfo.receivers.length; i++) {
        const numberRecieved = await recognition.countRecognitionsReceived(userInfo.receivers[i].id);
        results.push(
            bot.say({
