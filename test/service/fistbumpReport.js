...
describe("fistbumpReport service", () => {
  ...
  describe("getFistbumpChartData", () => {
    it("should fetch recognition data and transform it for chart display", async () => {
      ...
      // act
      const result = await fistbumpReport.getFistbumpChartData(
        timeRange,
        timezone,
      );
      ...
      // assert
      expect(
        mockRecognition.getPreviousXDaysOfRecognition.calledOnceWith(
          timezone,
          timeRange,
        ),
      ).to.be.true;
  ...
  describe("createFistbumpReportBlocks", () => {
    it("should create blocks for a Slack message with chart and stats", async () => {
      ...
      // act
      const blocks = await fistbumpReport.createFistbumpReportBlocks(
        timeRange,
        timezone,
      );
      ...
      // assert
      expect(
        fistbumpReport.getFistbumpChartData.calledOnceWith(timeRange, timezone),
      ).to.be.true;
      expect(blocks).to.be.an("array");
      expect(blocks.length).to.be.at.least(4); // should have header, context, image, and stats
      expect(blocks[0])
        .to.have.nested.property("text.text")
  ...
  describe("postFistbumpReport", () => {
    it("should post report blocks to the specified channel", async () => {
      ...
      const mockBlocks = [
        { type: "header", text: { type: "plain_text", text: "Test Report" } },
      ];
      sinon
        .stub(fistbumpReport, "createFistbumpReportBlocks")
        .resolves(mockBlocks);

      // act
      const result = await fistbumpReport.postFistbumpReport(
        mockClient,
        channelId,
        timeRange,
      );
      ...
      // assert
      expect(
        fistbumpReport.createFistbumpReportBlocks.calledOnceWith(timeRange),
      ).to.be.true;
      expect(mockClient.chat.postMessage.calledOnce).to.be.true;
      expect(mockClient.chat.postMessage.firstCall.args[0]).to.have.property(
        "channel",
        channelId,
      );
      expect(mockClient.chat.postMessage.firstCall.args[0]).to.have.property(
